...
inv_error_t inv_get_euler_angles_z(long *data)
{
    ...
    data[0] =
        (long)((float)
               (atan2f(rotMatrix[6], rotMatrix[7]) * 57.29577951308F) *
               65536L);
    data[1] = (long)((float)((double)asin(tmp) * 57.29577951308F) * 65536L);
    data[2] =
        (long)((float)
               (atan2f(rotMatrix[5], rotMatrix[2]) * 57.29577951308F) *
               65536L);
    return result;
}
...
inv_error_t inv_get_mag_bias(long *data)
{
    ...
    if (IS_INV_ADVFEATURES_ENABLED(inv_obj)) {
        data[0] += (long)((long long)inv_obj.adv_fusion->init_compass_bias[0]
                          * inv_obj.mag->sens / 16384);
        data[1] += (long)((long long)inv_obj.adv_fusion->init_compass_bias[1]
                          * inv_obj.mag->sens / 16384);
        data[2] += (long)((long long)inv_obj.adv_fusion->init_compass_bias[2]
                          * inv_obj.mag->sens / 16384);
    }
...
inv_error_t inv_get_heading(long *data)
{
    ...
    result = inv_get_rot_mat_float(rotMatrix);
    if ((rotMatrix[7] < 0.707F) && (rotMatrix[7] > -0.707F)) {
        tmp =
            (float)(atan2f(rotMatrix[4], rotMatrix[1]) * 57.29577951308F -
                    90.0F);
    } else {
        tmp =
            (float)(atan2f(rotMatrix[5], rotMatrix[2]) * 57.29577951308F +
                    90.0F);
    }
    if (tmp < 0) {
        tmp += 360.0F;
...
inv_error_t inv_get_mag_bias_float(float *data)
{
    ...
    if(IS_INV_ADVFEATURES_ENABLED(inv_obj)) {
        data[0] += (float)((long long)inv_obj.adv_fusion->init_compass_bias[0] 
                           * inv_obj.mag->sens / 16384) / 65536.0f;
        data[1] += (float)((long long)inv_obj.adv_fusion->init_compass_bias[1] 
                           * inv_obj.mag->sens / 16384) / 65536.0f;
        data[2] += (float)((long long)inv_obj.adv_fusion->init_compass_bias[2] 
                           * inv_obj.mag->sens / 16384) / 65536.0f;                           
    }
...
inv_error_t inv_get_euler_angles_x_float(float *data)
{
    ...
    data[0] =
        (float)(atan2f(rotMatrix[7],
                       rotMatrix[8]) * 57.29577951308F);
    data[1] = (float)((double)asin(tmp) * 57.29577951308F);
    data[2] =
        (float)(atan2f(rotMatrix[3], rotMatrix[0]) * 57.29577951308F);
...
inv_error_t inv_get_euler_angles_y_float(float *data)
{
    ...
    data[0] =
        (float)(atan2f(rotMatrix[8], rotMatrix[6]) * 57.29577951308F);
...
inv_error_t inv_get_euler_angles_z_float(float *data)
{
    ...
    data[0] =
        (float)(atan2f(rotMatrix[6], rotMatrix[7]) * 57.29577951308F);
    data[1] = (float)((double)asin(tmp) * 57.29577951308);
    data[2] =
        (float)(atan2f(rotMatrix[5], rotMatrix[2]) * 57.29577951308F);
...
inv_error_t inv_get_mag_raw_data_float(float *data)
{
    ...
    if (IS_INV_ADVFEATURES_ENABLED(inv_obj)) {
        data[0] += (float)inv_obj.adv_fusion->init_compass_bias[0];
        data[1] += (float)inv_obj.adv_fusion->init_compass_bias[1];
        data[2] += (float)inv_obj.adv_fusion->init_compass_bias[2];
    }
...
inv_error_t inv_set_mag_bias(long *data)
{
    ...
    if(IS_INV_ADVFEATURES_ENABLED(inv_obj)) {
        inv_obj.adv_fusion->init_compass_bias[0] = 0;
        inv_obj.adv_fusion->init_compass_bias[1] = 0;
        inv_obj.adv_fusion->init_compass_bias[2] = 0;

			  ...
			  struct mpu6050_config *config, long apply, long odr)
{
	...
	if (odr != 0) {
		...
		/* final odr MAY be differnt from requested odr due to
		   integer truncation */
	...
	if (apply
		...
		result = inv_serial_single_write(mlsl_handle, pdata->address,
					MPUREG_SMPLRT_DIV, total_divider);
		if (result) {
	...
	default:
		...
		if (b == BIT_CYCLE) {
			MPL_LOGI(" Accel LP - > FP mode. \n ");
				mpu6050_set_fp_mode(mlsl_handle,
							pdata);
		}
	}
...
#ifdef UMPL
static struct mpu6050_private_data mpu6050_private_data_instance;
#endif
...
#else
    private_data = &mpu6050_private_data_instance;
#endif

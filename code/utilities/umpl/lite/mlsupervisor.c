                                          ...
                                          unsigned long deltaTime)
{
    ...
    case CAL_RUN:
        ...
        for (i = 0; i < 3; i++) {
            ...
            inv_obj.adv_fusion->compass_test_bias[i] =
                -(long)(tmp * inv_obj.mag->sens / 16384.0f);
        }
        break;
    case CAL_RESET:
        for (i = 0; i < 4; i++) {
            ...
            if (compassCalStableCount < 1000) {
                ...
                for (i = 0; i < 3; i++) {
                    if (magMax[i] - magMin[i] <
                        (long long)40 * 1073741824LL / inv_obj.mag->sens) {
                        compassCalStableCount = 0;
                    }
                }
            } else {
                if (compassCalStableCount >= 1000) {
                    if (MLUpdateCompassCalibration3DOF
                        ...
                        inv_set_compass_bias(inv_obj.adv_fusion->compass_test_bias);
                    }
                    compassCalCount = 0;
                    compassCalStableCount = 0;
                    for (i = 0; i < 3; i++) {
...
inv_error_t inv_reset_compass_calibration(void)
{
    ...
    MLUpdateCompassCalibration3DOF(CAL_RESET, inv_obj.mag->sensor_data, 1);

    inv_obj.adv_fusion->compass_bias_error[0] = P_INIT;
    inv_obj.adv_fusion->compass_bias_error[1] = P_INIT;
    inv_obj.adv_fusion->compass_bias_error[2] = P_INIT;
    inv_obj.adv_fusion->compass_accuracy = 0;

    inv_obj.adv_fusion->got_compass_bias = 0;
    inv_obj.adv_fusion->got_init_compass_bias = 0;
    inv_obj.adv_fusion->compass_state = SF_UNCALIBRATED;
    inv_obj.adv_fusion->resetting_compass = 1;

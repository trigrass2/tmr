...
#ifndef UMPL_NINE_AXIS
#error UMPL_NINE_AXIS required when using compass.
#error mldl_cfg_init_umpl_6050.c uses compass.
#endif
...
#define ACCEL_GET_SLAVE_DESCR mpu6050_get_slave_descr
struct ext_slave_descr * ACCEL_GET_SLAVE_DESCR(void);
#define ACCEL_SLAVE_ADDRESS 0x68
...
#define COMPASS_GET_SLAVE_DESCR ak8975_get_slave_descr
struct ext_slave_descr * COMPASS_GET_SLAVE_DESCR(void);
#define COMPASS_SLAVE_ADDRESS 0x0E
#define COMPASS_ORIENT_00 0
#define COMPASS_ORIENT_01 1
#define COMPASS_ORIENT_02 0
#define COMPASS_ORIENT_10 1
#define COMPASS_ORIENT_11 0
#define COMPASS_ORIENT_12 0
#define COMPASS_ORIENT_20 0
#define COMPASS_ORIENT_21 0
#define COMPASS_ORIENT_22 -1


/*---- structure containing control variables used by MLDL ----*/
static struct mpu_ram s_mpu_ram = {
    /* __u16 length = */   DMP_CODE_SIZE
    /* __u8  *ram   = */ , &dmpMemory 
};
...
struct ext_slave_platform_data s_pdata_slave_accel = {
    ...
    /* __u32 irq            = */ , 0
    /* __u32 adapt_num      = */ , 0
    /* __u32 bus            = */ , EXT_SLAVE_BUS_PRIMARY
...
struct ext_slave_platform_data s_pdata_slave_compass = {
    /* __u8 type            = */   EXT_SLAVE_TYPE_COMPASS
    /* __u32 irq            = */ , 0
    /* __u32 adapt_num      = */ , 0
    /* __u32 bus            = */ , EXT_SLAVE_BUS_SECONDARY
    /* __u8 address         = */ , COMPASS_SLAVE_ADDRESS
    /* __s8 orientation[9]  = */ , 
        { COMPASS_ORIENT_00 , COMPASS_ORIENT_01 , COMPASS_ORIENT_02
        , COMPASS_ORIENT_10 , COMPASS_ORIENT_11 , COMPASS_ORIENT_12
        , COMPASS_ORIENT_20 , COMPASS_ORIENT_21 , COMPASS_ORIENT_22 }
    /* void *irq_data       = */ , NULL
    /* void *private_data   = */ , NULL
};
...
inv_error_t inv_mldl_cfg_init(struct mldl_cfg **p_g_mldl_cfg)
{
    ...
    s_mldl_cfg.slave[EXT_SLAVE_TYPE_COMPASS]  = COMPASS_GET_SLAVE_DESCR();
